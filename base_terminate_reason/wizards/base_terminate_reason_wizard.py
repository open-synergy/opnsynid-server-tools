...
class BaseTerminateReasonWizard(models.TransientModel):
    ...
    def fields_view_get(
        ...
        terminate_reason_ids = self._get_terminate_reason_ids(active_model)
    ...
    def _get_terminate_reason_ids(self, model):
        obj_terminate_reason_config = self.env["base.terminate.reason_config"]
        criteria = [("name", "=", model)]
        terminate_reason_config_id = obj_terminate_reason_config.search(criteria)
    ...
    def _confirm_terminate(self):
        ...
        active_model = self.env.context.get("active_model", "")
        object_id = self.env[active_model].browse(active_ids)
        method_terminate_name = self._get_method_terminate_name(active_model)
        if method_terminate_name and hasattr(object_id, method_terminate_name):
            method_terminate = getattr(object_id, method_terminate_name)
            method_terminate()
            if hasattr(object_id, "terminate_reason_id"):
                object_id.write(self._prepare_terminate_reason_data())
        else:
            return True

    @api.model
    def _prepare_terminate_reason_data(self):
        result = {"terminate_reason_id": self.terminate_reason_id.id}
        return result
