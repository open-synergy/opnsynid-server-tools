...
class BaseCancelReasonWizard(models.TransientModel):
    ...
    def fields_view_get(
        ...
        cancel_reason_ids = self._get_cancel_reason_ids(active_model)
    ...
    def _get_cancel_reason_ids(self, model):
        obj_cancel_reason_config = self.env["base.cancel.reason_config"]
        criteria = [("name", "=", model)]
        cancel_reason_config_id = obj_cancel_reason_config.search(criteria)
    ...
    def _confirm_cancel(self):
        ...
        method_cancel_name = self._get_method_cancel_name(active_model)
        if method_cancel_name and hasattr(object_id, method_cancel_name):
            method_cancel = getattr(object_id, method_cancel_name)
            method_cancel()
            if hasattr(object_id, "cancel_reason_id"):
                object_id.write(self._prepare_cancel_reason_data())
        else:
            return True

    @api.model
    def _prepare_cancel_reason_data(self):
        result = {"cancel_reason_id": self.cancel_reason_id.id}
        return result
