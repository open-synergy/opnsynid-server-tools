...
class BaseAmountToText(models.Model):
    ...
    lang_id = fields.Many2one(
        comodel_name="res.lang", string="Languange", ondelete="cascade"
    )
    currency_id = fields.Many2one(
        comodel_name="res.currency", string="Currency", ondelete="cascade"
    )
    ...
    @api.model
    def get(self, value, currency, lang=False):
        user = self.env.user
        if not lang:
            criteria = [("code", "=", user.partner_id.lang)]
            lang = self.env["res.lang"].search(criteria, limit=1)
        att = self.search(
            [
                ("lang_id", "=", lang.id),
